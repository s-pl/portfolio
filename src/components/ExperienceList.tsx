...
function CaseStudyBlock({
  ...
  return (
    <details
      className="group mt-4 border-l border-emerald-400/30 pl-4"
      onToggle={(event) => setOpen(event.currentTarget.open)}
    >
      <summary className="flex cursor-pointer list-none items-center gap-2 rounded-sm font-mono text-sm text-emerald-400 transition-colors hover:text-emerald-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring [&::-webkit-details-marker]:hidden">
        <span aria-hidden="true">{open ? "−" : "+"}</span>
        <span className="group-open:hidden">{labels.open}</span>
        <span className="hidden group-open:inline">{labels.close}</span>
        <span className="text-muted-foreground/50">· {caseStudy.title}</span>
      </summary>
          ...
          >
            ...
            <div className="pb-2">
              <div className="flex flex-wrap items-center gap-2 mb-0.5">
                <span className="font-mono text-base font-semibold">{exp.role}</span>
                {exp.tag && (
              ...
              <p className="text-sm text-muted-foreground/70 font-mono mb-2">
              ...
              {exp.tech && exp.tech.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {exp.tech.map((t) => (
                    <span
                      key={t}
                      className="font-mono text-sm text-muted-foreground/60 border border-border rounded px-1.5 py-0.5"
                    >
                      {t}
                    </span>
                  ))}
